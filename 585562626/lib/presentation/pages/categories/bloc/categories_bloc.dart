...
class CategoriesBloc extends Bloc<CategoriesEvent, CategoriesState> {
  ...
  CategoriesBloc({
    CategoriesState initialState = const InitialCategoriesState(),
  ...
  Stream<CategoriesState> _addCategory(
    ...
    yield CategoriesFetchedState(categories);
    final result = await categoriesUseCase.addCategory(event.category);
    if (result == 0) {
  ...
  Stream<CategoriesState> _update(
    ...
    yield CategoriesFetchedState(categories);
    final result = await categoriesUseCase.updateCategory(category);
    if (result == 0) {
  ...
  Stream<CategoriesState> _deleteCategory(
    ...
    categories.remove(event.category);
    yield CategoriesFetchedState(categories);
    final result = await categoriesUseCase.deleteCategory(event.category);
    if (result == 0) {
