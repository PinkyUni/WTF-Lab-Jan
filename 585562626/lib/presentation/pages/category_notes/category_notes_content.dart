...
class CategoryNotesContent extends StatefulWidget {
  ...
  const CategoryNotesContent({Key? key, required this.category}) : super(key: key);
...
class _CategoryNotesContentState extends State<CategoryNotesContent> {
  late final CategoryNotesBloc _bloc = injector.get(param1: widget.category);
  ...
  void _showSearch(CategoryNotesState state) async {
    await showSearch(
      context: context,
      delegate: NoteSearch(context, state.notes, state.tags),
    );
